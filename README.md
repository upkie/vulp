...
* [ros2_control](https://github.com/ros-controls/ros2_control) - A C++ framework for real-time control using ROS2 (still a work in progress). Its barrier of entry is steeper than the other alternatives, making it suitable for production rather than prototyping. Extends URDF with a ``<ros2_control>`` tag to describe actuators/sensors and link them to dynamically-loaded shared libraries.
