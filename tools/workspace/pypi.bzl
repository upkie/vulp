...
def pypi_download_and_extract(
        ...
        strip_prefix = ""):
    """
    Downloads an archive of the provided PyPI package and version to the output
    path and extracts it.
    ...
    Args:
        ...
        sha256: expected SHA-256 hash of the archive downloaded. Fallback to an
            incorrect default value to prevent the hash check from being
...
def _sha256(sha256):
    """
    Fallback to an incorrect default value of SHA-256 hash to prevent the hash
    check from being disabled, but allow the first download attempt of an
...
def _strip_prefix(package, version, strip_prefix = ""):
    """
    Computes the strip prefix for a downloaded archive of the provided PyPI
    package and version.
